...
class EIVA_Sequence(SequenceBase[StereoInertialFrame]):
    ...
    @classmethod
    def name(cls) -> str: return "EIVA"

    def __init__(self, config: SimpleNamespace | dict[str, Any]):
        cfg = self.config_dict2ns(config)
        self.stereo_sequence = EIVA_StereoSequence(cfg)

        if cfg.imu_freq is None:
            self.imu_sequence    = EIVAIMULoader(Path(cfg.root, "imu"))
        else:
            self.imu_sequence    = EIVAIMUSimulator(cfg.imu_sim, Path(cfg.root, "pose_gt.txt"), fps=cfg.imu_freq)
...
class EIVA_StereoSequence(SequenceBase[StereoFrame]):
    ...
    def __init__(self, config: SimpleNamespace | dict[str, Any]):
        ...
        self.lcam_K    = torch.tensor([
            [419.9070, 0.0, 316.2046],
            [0.0, 314.9302, 239.8597],
            [0.0, 0.0, 1.0]]).unsqueeze(0)
...
class EIVAIMULoader(Dataset[tuple[IMUData, AttitudeData]]):
...
class EIVAIMUSimulator(Dataset[tuple[IMUData, AttitudeData]]):
