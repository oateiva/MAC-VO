...
def load_poses_from_txt_subpipe(file_name: str):
    """Load absolute camera poses from txt file (SubPipe-style) as pp.SE3.
...
class SubPipeSequence(SequenceBase[Frame]):
    @classmethod
    def name(cls) -> str: return "SubPipe"
    ...
    def __init__(self, config: SimpleNamespace | dict[str, Any]):
        ...
        # Loaders
        self.lcam_loader = SubPipeMonocularDataset(
            Path(cfg.root, "Cam0_images"),
            window_length=self.window_length,
            step_size=self.step_size,
            undistort=np.array([-0.28340811, 0.07395907, 0.00019359, 1.76187114e-05, 0.0]),
            K=self.lcam_K.squeeze(0).numpy(),
            )

        if self.is_stereo:
            assert ("Subpipe does not provide stereo images, please set is_stereo to false in config if you wish to use this dataset")
        else:
        ...
        poses_ts = load_poses_from_txt_subpipe(Path(cfg.root, "EstimatedState.csv"))
...
class SubPipeMonocularDataset(Dataset):
    """
    ...
    """
    def __init__(self, directory: Path, window_length: int, step_size: int = 1, undistort: np.ndarray | None = None, K: np.ndarray | None = None) -> None:
        super().__init__()
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        ...
        for i in range(self.window_length):
             ...
             if self.distort_factor is not None:
    ...
    def correct_distortion(self, image: np.ndarray) -> np.ndarray:
        if self.distort_factor is None:
...
def loadSubPipeGT(path: Path) -> pp.LieTensor:
